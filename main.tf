...
resource "aws_ecr_lifecycle_policy" "private" {
  for_each = {
    ...
    name => config if contains(keys(config), "max_untagged_image_count") || contains(keys(config), "max_tagged_image_count")
  }
...
resource "aws_ecrpublic_repository" "public" {
 ...
 tags = merge(
  ...
  dynamic "catalog_data" {
    for_each = contains(keys(each.value), "catalog_data") ? [each.value.catalog_data] : []
